# Hub for dlos network

This is a node for dlos network that serves as [relay](../../../dlos-relay), plus facilitates exchange of end-to-end encrypted messages among devices connected to dlos network.  The hub does not hold any private keys and cannot send payments itself.
...
* Plain text chat with bots that offer a service and can receive or send payments.  [Faucet](../../../dlos-faucet) is an example of such bot.
...
Users set their hub address in their wallet settings.  The default hub is wss://dlos.cn/bb but users can change it to your hub address.
...
npm install
```
Set up a proxy, such as nginx, to forward all websocket connections on a specific path to your daemon running the hub code.  See example configuration for nginx in [dloscore](../../../dloscore) documentation.
...
You need a private key generated by dlos Team for you, key_id and team_id to communicate with APNS servers on behalf of dlos app bundle, email us for ones: dlos at dlos.cn, then configure your hub (conf.js or conf.json) to enable notifications.
